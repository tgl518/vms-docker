...
const columns = [
    ...
    render: ({ roles }) => {
      if (roles?.length) {
        return roles.map((roleCode, index) => {
          // 角色编码转显示名称
          const roleNames = { 'SUPER_ADMIN': '超级管理员', 'admin': '管理员', 'user': '普通用户' }
          return h(
            NTag,
            { type: 'success', style: index > 0 ? 'margin-left: 8px;' : '' },
            { default: () => roleNames[roleCode] || roleCode },
          )
        })
      }
      return '暂无角色'
  ...
  },
  {
    ...
    render(row) {
      ...
      // 通过角色ID匹配角色编码
      const roleCodeMap = { 1: 'SUPER_ADMIN', 2: 'user', 3: 'admin' }
      const targetRoleCode = roleCodeMap[roleId]
      const hasRole = row.roles?.includes(targetRoleCode)
